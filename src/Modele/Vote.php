...
class Vote
{
    ...
    function rating_bar($pageSlug, $units='', $static='') {
        ...
        if ($static == 'static') {
            ...
            for ($ncount = 1; $ncount <= $units; $ncount++) { // loop from 1 to the number of units
                if(!$voted) { // if the user hasn't yet voted, draw the voting stars
                    $rater.='<li><a href="/vote/vote/db.php?j='.$ncount.'&amp;q='.$page_id.'&amp;t='.$ip.'&amp;c='.$units.'" title="'.$ncount.' sur '.$units.'" class="r'.$ncount.'-unit rater" rel="nofollow">'.$ncount.'</a></li>';
                }
            }
